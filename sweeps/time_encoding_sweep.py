...
def apply_time_encoding(df, encoding, phases=None):
    max_time = np.log(df['Timepoint'].max())

    if encoding == 'raw':
        df['time_enc'] = df['Timepoint']

    elif encoding == 'log':
        df['time_enc_log'] = np.log(df['Timepoint'])
        ...
        df['time_sin'] = np.sin(2 * np.pi * np.log(df['Timepoint']) / max_time)
...
def train():
    ...
    if accelerator.is_main_process:
        run = wandb.init(project="GrowthCurve Time Encoding Benchmark")
    else:
    ...
    df_train = apply_time_encoding(df_train, config.encoding, phases=config.phases)
    df_test  = apply_time_encoding(df_test, config.encoding, phases=config.phases)

    df_test_time_4_16 = df_test[df_test['Timepoint'] == 4.16]
    df_test_time_10_4 = df_test[df_test['Timepoint'] == 10.4]

    cols_rem = ['Well','Plate_ID','Compound','Control_Label','Smiles','is_Active','scaffold','OD','Timepoint']
    ...
    model = SimpleMLP(input_dim=X_train_tensor.shape[1], output_dim=1, hidden_dim=config.hidden_dim, num_layers=config.hidden_layers,
    ...
    for epoch in range(config.epochs):
        ...
        with torch.no_grad():
            ...
            mae_val_t_4_16 = unweighted_metric(model, df_test_time_4_16, cols_rem, mean_absolute_error)
            pearson_val_t_4_16 = unweighted_metric(model, df_test_time_4_16, cols_rem, pearson_np)

            mae_val_t_10_4 = unweighted_metric(model, df_test_time_10_4, cols_rem, mean_absolute_error)
            pearson_val_t_10_4 = unweighted_metric(model, df_test_time_10_4, cols_rem, pearson_np)

            Pearson_MAE = pearson_val - mae_val

            current_metric = Pearson_MAE    # whatever you called it above
        ...
        if accelerator.is_main_process: 
            wandb.log({
                ...
                "val_mae_time_4_16": mae_val_t_4_16,
                "pearson_val_time_4_16": pearson_val_t_4_16,

                "val_mae_time_10_4": mae_val_t_10_4,
                "pearson_val_time_10_4": pearson_val_t_10_4,



                "Pearson_val - MAE_val" : Pearson_MAE,
